...
export function MarketplaceNav() {
  ...
  return (
    <>
      <header className="palmer-nav-shell sticky top-0 z-50 border-b border-black/10 bg-[color:var(--paper)]/94 backdrop-blur-2xl supports-[backdrop-filter]:bg-[color:var(--paper)]/82">
        <div className="mx-auto flex min-h-18 max-w-[1800px] items-center justify-between gap-4 px-5 lg:px-10">
          <Link href="/" className="group flex min-h-11 shrink-0 items-center gap-3" aria-label="WhisperX Market home">
            <span className="palmer-wordmark grid size-10 place-items-center font-display text-2xl leading-none text-[color:var(--signal)]">X</span>
            <span>
              <span className="block text-sm font-semibold tracking-[-0.03em]">WHISPERX®</span>
              <span className="ui-eyebrow mt-0.5 block text-[color:var(--muted-foreground)]">systems / craft</span>
            </span>
          </Link>
            ...
            {navItems.map((item, index) => {
              ...
              return (
                <Link
                  ...
                  className={`group flex min-h-11 items-center gap-2 font-mono text-[11px] font-semibold uppercase tracking-[0.16em] transition ${active ? "text-foreground" : "text-foreground/52 hover:text-foreground"}`}
                >
                  <span className="text-[9px] text-foreground/28">{String(index + 1).padStart(2, "0")}</span>
                  <span className="palmer-rolling-link">
          ...
          <div className="flex items-center gap-2">
            <Link
              ...
              className="grid size-11 place-items-center border border-black/10 bg-[color:var(--surface-raised)] text-foreground/60 transition hover:-translate-y-0.5 hover:border-[color:var(--signal)] hover:text-[color:var(--signal)] hover:shadow-lg"
          ...
          {navItems.map(({ href, label, shortLabel, icon: Icon }, index) => {
            ...
            return (
              <Link
                ...
                aria-label={label}
                aria-current={active ? "page" : undefined}
                className={`relative flex min-h-14 min-w-[104px] flex-1 items-center justify-center gap-2 border-r border-black/5 px-3 font-mono text-[11px] font-semibold uppercase tracking-[0.1em] last:border-r-0 ${active ? "bg-[color:var(--signal-soft)] text-[color:var(--signal)]" : "text-foreground/48"}`}
              >
